 ...
 */
export const searchDentixVectors = async (query) => {
 ...
 * @returns Array de resultados de la búsqueda vectorial
 */
export const searchCredintegralVectors = async (query) => {
    const embeddings = createEmbeddings();
    const supabase = createSupabaseClient();
 ...
 */
export const searchVidaDeudorVectors = async (query) => {
    const embeddings = createEmbeddings();
    const supabase = createSupabaseClient();
    ...
    try {
        ...
        if (isFarmaciaQuery) {
            const farmaciaDoc = allChunks.find((doc) => doc.id === 35 ||
            ...
            if (farmaciaDoc) {
                ...
                const otherDocs = allChunks.filter((doc) => doc.id !== farmaciaDoc.id);
 ...
 */
export const searchVidaDeudorTariffs = async (query) => {
    const supabase = createSupabaseClient();
 ...
 * @returns Array de resultados de la búsqueda vectorial
 */
export const searchBienestarVectors = async (query) => {
    const embeddings = createEmbeddings();
    const supabase = createSupabaseClient();
    ...
    try {
        ...
        for (const searchQuery of searchQueries) {
            ...
            if (data && data.length > 0) {
                ...
                console.log(`[DEBUG] Se recibieron ${data.length} chunks. IDs:`, data.map((d) => d.id));
 ...
 * @returns Array de resultados de la búsqueda vectorial
 */
export const searchSoatVectors = async (query) => {
    const supabase = createSupabaseClient();
    const embeddings = createEmbeddings();
    ...
    try {
        ...
        const { data: vectorResults, error: vectorError } = await supabase.rpc('match_soat_documents', // Función RPC que debe existir en Supabase
        {
 ...
 */
async function fallbackTextSearchSoat(query) {
    const supabase = createSupabaseClient();
    ...
    try {
        ...
        if (isFineQuery) {
            ...
            searchTerms.push('consecuencias', 'Consecuencias de no tener SOAT', 'Consecuencias de no tener SOAT vigente', 'multa', 'sanción', 'penalidad', 'obligatorio', 'circular sin SOAT');
        }
        else {
 ...
 * @returns Array de resultados de la búsqueda vectorial
 */
export const searchMascotaVectors = async (query) => {
    const embeddings = createEmbeddings();
    const supabase = createSupabaseClient();
