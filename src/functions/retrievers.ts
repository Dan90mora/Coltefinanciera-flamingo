 ...
 * @returns Array de resultados de la búsqueda vectorial
 */
export const searchAutosVectors = async (query: string): Promise<any[]> => {
    const embeddings = createEmbeddings();
    const supabase = createSupabaseClient();
    ...
    try {
        ...
        if (isPriceQuery && allChunks.length > 0) {
            const priceChunk = allChunks.find(chunk => 
                /marca.*modelo.*año|modelo.*año.*marca|información.*vehículo|datos.*vehículo|cotización/i.test(chunk.content)
            );
            if (priceChunk) {
