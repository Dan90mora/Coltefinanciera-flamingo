...
export async function saveChatHistory(clientNumber: string, newMessage: string, isClient: boolean, newMediaUrl: string): Promise<void> {
    try {
...
// Función para guardar o actualizar el historial del chat
export async function saveTemplateChatHistory(clientNumber: string, newMessage: string, isClient: boolean, newMediaUrl: string, user: string): Promise<void> {
    try {
        const firebaseMediaUrl = newMediaUrl ? newMediaUrl : '';
        // Verificar si el cliente ya tiene un chat
        const { data: existingChat, error: fetchError } = await supabase
        ...
        const newEntry: updateWithTemplate = {
            user: user,
            message: newMessage,
            url: firebaseMediaUrl,
            date: new Date().toISOString()
        ...
        if (existingChat) {
            // Si el cliente ya tiene un chat, agregar el nuevo mensaje al historial existente
            const updatedMessages: updateWithTemplate[] = [...existingChat.messages, newEntry];
            ...
            // Si el cliente no tiene un chat, crear un nuevo registro con el historial inicial
            const updatedMessages: updateWithTemplate[] = [newEntry];
