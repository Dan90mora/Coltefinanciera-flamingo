...
export const vehicleServiceNode = async (
    ...
    const extractDataFromMessage = (message: string) => {
      ...
      for (const pattern of yearPatterns) {
        ...
        if (extractedData.vehicleYear) break;
      }
    ...
    if (hasPurchaseIntent || isProvidingData || isProvidingVehicleData) {
      ...
      const missingData = [];
      if (!state.vehicleInsuranceData.birthDate) missingData.push('Fecha de nacimiento (DD/MM/YYYY)');
      if (!state.vehicleInsuranceData.vehicleBrand) missingData.push('Marca del vehículo');
      if (!state.vehicleInsuranceData.vehicleModel) missingData.push('Modelo del vehículo');
      if (!state.vehicleInsuranceData.vehicleYear) missingData.push('Año del vehículo');
      if (!state.vehicleInsuranceData.vehiclePlate) missingData.push('Placa del vehículo');
      if (!state.vehicleInsuranceData.vehicleCity) missingData.push('Ciudad de circulación del vehículo');
      ...
      if (missingData.length > 0) {
        ...
        if (Object.values(extractedData).some(val => val)) {
          ...
          if (extractedData.fullName) message += `• Nombre: ${extractedData.fullName}\n`;
          if (extractedData.cedula) message += `• Cédula: ${extractedData.cedula}\n`;
          if (extractedData.birthDate) message += `• Fecha de nacimiento: ${extractedData.birthDate}\n`;
          if (extractedData.phone) message += `• Teléfono: ${extractedData.phone}\n`;
          if (extractedData.vehicleBrand) message += `• Marca: ${extractedData.vehicleBrand}\n`;
          if (extractedData.vehicleModel) message += `• Modelo: ${extractedData.vehicleModel}\n`;
          if (extractedData.vehicleYear) message += `• Año: ${extractedData.vehicleYear}\n`;
          if (extractedData.vehiclePlate) message += `• Placa: ${extractedData.vehiclePlate}\n`;
          if (extractedData.vehicleCity) message += `• Ciudad: ${extractedData.vehicleCity}\n`;
          message += '\n';
        }
