...
import { AgentState, MascotaInsuranceData } from "./agentState";
...
export const mascotaServiceNode = async (
    ...
    const extractDataFromMessage = (message: string) => {
      ...
      for (const pattern of petTypePatterns) {
        ...
        if (petTypeMatch && petTypeMatch[1]) {
          ...
          // Normalizar tipos
          if (petType === 'perrita') petType = 'perro';
          if (petType === 'gata' || petType === 'gatito' || petType === 'gatita') petType = 'gato';
    ...
    if (isPurchaseIntent) {
      ...
      const updatedMascotaData: MascotaInsuranceData = {
      ...
      if (!updatedMascotaData.fullName) missingData.push('**nombre completo**');
      if (!updatedMascotaData.birthDate) missingData.push('**fecha de nacimiento**');
      
      // Datos específicos de la mascota
      if (!updatedMascotaData.petName) missingPetData.push('**nombre de tu mascota**');
      if (!updatedMascotaData.petType) missingPetData.push('**tipo de mascota** (perro, gato, etc.)');
      if (!updatedMascotaData.petAge) missingPetData.push('**edad de tu mascota**');
      if (!updatedMascotaData.city) missingPetData.push('**ciudad donde vives**');
